    ...
    def connect(self,message,**kwargs):
        ...
        if self.message.user:
            ...
            try:
                ...
                message.reply_channel.send({"text": json.dumps(['stdout', '\033[1;3;31m"主机不存在或者已被删除"\033[0m'])},
                                           immediately=True)
            ...
            try:
                if user.is_superuser:
                    ...
                    user_server = User_Host.objects.get(user_id=user.id, server_id=sid)
            ...
            try:
                ...
                self.ssh.connect(server.ip, port=int(server.port), username=server.username, password=password,
                                 timeout=3, pkey=pkey)
            except socket.timeout:
                message.reply_channel.send(
                    {"text": json.dumps(['stdout', '\033[1;3;31mConnect to server time out\033[0m'])}, immediately=True)
                message.reply_channel.send({"accept": False})
                self.disconnect(message)
                return
            except Exception, ex:
                message.reply_channel.send(
                    {"text": json.dumps(['stdout', '\033[1;3;31m连接服务器失败: {ex}\033[0m'.format(ex=str(ex))])},
                    immediately=True)
                message.reply_channel.send({"accept": False})
                self.disconnect(message)
                return
