...
from CMDB.models import OracleCluster,Oracle_Instance
from CMDB.serializers import OracleClusterSerializer,OracleInstanceSerializer
...
# 分页功能的
class OracleClusterAPI(APIView):
    """
     列出所有出版社或者创建一个 新的出版社
      """
    ...
    def get_object(self, id):
        try:
            return OracleCluster.objects.get(id=id)
        except OracleCluster.DoesNotExist:
            raise Http404
    ...
    def get(self, request, format=None):
            ...
            s = OracleClusterSerializer(instance=page_data, many=True)  # 序列花这个分页数据
    ...
    def post(self, request, format=None):
        s = OracleClusterSerializer(data=request.data)
        if s.is_valid():  # 验证
            s.save()
    ...
    def put(self, request, format=None):
        try:
            ...
            s = OracleClusterSerializer(DATA_MODEL, data=request.data)
        ...
        try:
            ...
            try:
                for i in data_id:
                    ...
                    # else:
                    OracleCluster.objects.filter(id=i).delete()
...
# 实例功能的
class OracleInstanceAPI(APIView):
    """
     列出所有出版社或者创建一个 新的出版社
      """
    # permission_classes = (AuthOrReadOnly)
    ...
    def get_object(self, id):
        try:
            return Oracle_Instance.objects.get(id=id)
        except Oracle_Instance.DoesNotExist:
            raise Http404
        ...
        try:
            ...
            try:
                for i in data_id:
                    ...
                    #     SqlOrder.objects.filter(id=i['id']).delete()
                    # else:
                    Oracle_Instance.objects.filter(id=i).delete()
                json_data = {'code': 200, 'msg': '删除成功'}
                return Response(json_data)
            except Exception as e:
                json_data = {'code': 500, 'msg': '删除失败'+e }
...
# 用户功能的
class OracleTablespaceAPI(APIView):
    """
     列出所有出版社或者创建一个 新的出版社
      """
    # permission_classes = (AuthOrReadOnly)
