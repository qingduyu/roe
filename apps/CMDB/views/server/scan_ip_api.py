...
from CMDB.serialize.ip_serializers import IP_Serializer
...
class IPAPI(APIView):
    ...
    def get_object(self, id):
        try:
            return IpSource.objects.get(id=id)
        except IpSource.DoesNotExist:
            raise Http404

    def get(self, request, format=None):

            queryset = IpSource.objects.all()
            ...
            s = IP_Serializer(instance=page_data, many=True)  # 序列花这个分页数据
    ...
    def post(self, request, format=None):
        s = IP_Serializer(data=request.data)
        if s.is_valid():  # 验证
            s.save()
    ...
    def put(self, request, format=None):
        try:
            ...
            s = IP_Serializer(DATA_MODEL, data=request.data)
        ...
        try:
            ...
            try:
                for i in data_id:
                    ...
                    # else:
                    IpSource.objects.filter(id=i).delete()
